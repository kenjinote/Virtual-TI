...
extern Z80_Regs R;
static int keymap82[]={VK_ESCAPE,k_esc,'1',k_1,'2',k_2,
...
char* defExt82[0x20]={"82n","82l","82m","82y","82p",
    "82p","82p","82i","82d","82p","82p","82w","82z",
    "82t","82p","82b"};

ROMFunc romFuncs82[]=
   {{0x13738,"TR_CHARPUT"},{0x139d2,"TX_CHARPUT"},
    {0x138fa,"D_ZT_STR"},{0x1373e,"D_LT_STR"},
    {0x1387c,"D_HL_DECI"},{0x13774,"LAST_LINE"},
    {0x1377a,"NEXT_LINE"},{0x13786,"SCROLL_UP"},
    {0x137f2,"UP_TEXT"},{0x137ce,"M_CHARPUT"},
    {0x137d4,"D_ZM_STR"},{0x137da,"D_LM_STR"},
    {0x1389a,"CLEARLCD"},{0x137b0,"CLEARTEXT_W"},
    {0x137a4,"CLEARTEXT_F"},{0x138ca,"CLEAR_DISP"},
    {0x139c6,"BACKUP_DISP"},{0x138dc,"RESTOR_DISP"},
    {0x138ac,"DISP_GRAPH"},{0x13924,"KEY_HAND"},
    {0x13918,"BUSY_OFF"},{0x13930,"BUSY_ON"},
    {0x1386a,"DONE"},{0,NULL}};
...
void TI82::EnableDebug()
{
}

void TI82::DisableDebug()
{
}
...
int TI82::GetKeyMat(int r,int c)
{
    return ti82kmat[r][c];
}

int TI82::GetKeyAlphaMat(int r,int c)
{
    return ti82alphakmat[r][c];
}
...
int TI82::getmem_word(int addr)
{
	return ((getmem(addr+1)<<8)|getmem(addr))&0xffff;
}

int TI82::getmem_dword(int addr)
{
...
void TI82::setmem(int addr,int v)
{
    addr&=0xffffff;
	if ((word)addr>=0x8000)
		ram[addr&0x7fff]=v;
...
void TI82::setmem_word(int addr,int v)
{
	setmem(addr+1,(v>>8)&0xff);
	setmem(addr,v&0xff);
}

void TI82::setmem_dword(int addr,int v)
{
...
int TI82::readmem_word(int addr)
{
    if (debugCheckEnable)
    {
...
int TI82::readmem_dword(int addr)
{
    if (debugCheckEnable)
    {
...
void TI82::writemem(int addr,int v)
{
    if (debugCheckEnable)
    {
...
void TI82::writemem_word(int addr,int v)
{
    if (debugCheckEnable)
    {
...
void TI82::writemem_dword(int addr,int v)
{
    if (debugCheckEnable)
    {
...
void TI82::writeport(int port,int v)
{
...
void TI82::KeyUp(int r,int c)
{
    if (r!=-1)
        kbmatrix[r]|=1<<c;
}

void TI82::KeyDown(int r,int c)
{
    if (r!=-1)
        kbmatrix[r]&=~(1<<c);
}
...
int TI82::GetKeyID(int i)
{
    return key[i];
}

void TI82::SaveState(char *fn)
{
    FILE *fp=fopen(fn,"wb");
    fprintf(fp,"VTIv2.0 ");
...
void TI82::LoadState(char *fn)
{
    ...
    fread(&contrast,4,1,fp); fread(&port2,4,1,fp);
    fclose(fp);
}

int TI82::SendByte(int c)
{
    int bit=1;
    int left;
...
int TI82::SendBuffer(int c)
{
    return SendByte(c);
}

int TI82::CheckBuffer()
{
...
int TI82::SendFile(char *fn,int more)
{
    ...
    while (!feof(fp))
    {
        ...
        if (type==0xf)
        {
            ...
            if (!GetByte(c)) { fclose(fp); sendInProgress=0; return 0; }
            whiteWire=1; redWire=1; KBInt();
            SendByte(0x82); SendByte(0x56); SendByte(0); SendByte(0);
            SendByte(0x82); SendByte(0x15);
            SendByte(len&0xff); SendByte(len>>8);
            fseek(fp,2,SEEK_CUR);
            csum=0;
            for (i=0;i<len;i++)
    ...
    if (!more)
    {
        SendByte(0x82); SendByte(0x92); SendByte(0); SendByte(0);
        WaitForAck();
    }
    fclose(fp);
    sendInProgress=0;
...
int TI82::ReceiveFile()
{
    ...
    while (1)
    {
        ...
        if (!transferAsGroup)
        {
            ...
            dlog->DefaultExt=defExt82[type&0xf];
            if (name[0]==0x5c)
