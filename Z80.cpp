...
extern int debugProgBreakOfs,debugProgLen;
...
extern int endStepOver,previouspc,ignoreStack;
...
/****************************************************************************/
void Interrupt (void/*int j*/)	/* NS 970904 */
{
...
int Z80_Execute(int cycles)	/* NS 970904 */
{
 ...
 do
 {
    if ((emuLink)&&(CheckSendBuf())&&(!sendInProgress))
    {
        sendInProgress=1;
        hw->SendByte(GetSendBuf());
        sendInProgress=0;
    }
    oldPC=R.PC.D;
#ifdef Z80_DAISYCHAIN
...
{	/* NS 971024 */
  ...
  R.PC.W.l++;
  oldICount=Z80_ICount;
  Z80_ICount-=cycles_main[opcode];
  (*(opcode_main[opcode]))();
        if (soundEnable)
            SoundOutBits(hw->outWhite,hw->outRed,oldICount-Z80_ICount);
        if (updateLink)
        {
            updateLink=0;
            EmuWnd->DoLink();
        ...
        if (debugProgBreak)
        {
            if ((calc==86)&&(R.PC.D==0xd748))
            {
                int len=hw->getmem_word(debugProgBreak)-2;
                for (i=0;i<len;i++)
                {
                    if (hw->getmem(debugProgBreak+4+i)!=
                        hw->getmem(0xd748+i))
                        break;
                }
                if (i!=len) continue;
            ...
            else if ((calc==82)&&((R.PC.D>=0x9104)&&(R.PC.D<=0x9124)))
            {
                int len=debugProgBreak&0xffff;
                int csum=0;
                for (int i=0x9104;i<(0x9104+len-5);i++)
                    csum+=hw->getmem(i);
                if ((csum&0xffff)!=((debugProgBreak>>16)&0xffff))
                    continue;
                debugProgBreak=0;
                run=0;
                debug=1;
            ...
            else if ((calc==83)&&(R.PC.D==0x9327))
            {
                int len=debugProgBreak&0xffff;
                int csum=0;
                for (int i=0x9327;i<(0x9327+len-2);i++)
                    csum+=hw->getmem(i);
                if ((csum&0xffff)!=((debugProgBreak>>16)&0xffff))
                    continue;
                debugProgBreak=0;
            ...
            else if ((calc==84)&&(R.PC.D==0x9d95))
            {
                int len=debugProgBreak&0xffff;
                int csum=0;
                for (int i=0x9d95;i<(0x9d95+len-4);i++)
                    csum+=hw->getmem(i);
                if ((csum&0xffff)!=((debugProgBreak>>16)&0xffff))
                    continue;
                debugProgBreak=0;
            ...
            else if ((calc==85)&&((R.PC.D>=0x8e57)&&
                (R.PC.D<=0x8e77)))
            {
                int len=debugProgLen;
                int csum=0;
                for (int i=0x8e57;i<(0x8e57+len-5);i++)
                    csum+=hw->getmem(i);
                if ((csum&0xffff)!=((debugProgBreak>>16)&0xffff))
                    continue;
                debugProgBreak=0;
            ...
            else if (calc==85)
            {
                ...
                if ((R.PC.D>=addr)&&(R.PC.D<=(addr+0x20)))
                {
                    int len=hw->getmem_word(addr);
                    if (debugProgLen!=len) continue;
                    debugProgBreak=0;
                    run=0;
                    debug=1;
                    debugStartPC=R.PC.D;
        ...
        if (debugStepOver)
        {
            ...
            if (debugStepOver&&(R.PC.D==debugStepGoal)&&
                ((R.SP.D==debugStepA7)||(ignoreStack)))
            {
                run=0;
                debug=1;
                debugStartPC=R.PC.D;
