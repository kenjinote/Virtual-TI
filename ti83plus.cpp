...
char* defExt8xp[0x20]={"8xn","8xl","8xm","8xy","8xp",
    "8xp","8xp","8xi","8xd","8xp","8xp","8xw","8xz",
    "8xt","8xp","8xb"};
...
void TI83Plus::EnableDebug()
{
}

void TI83Plus::DisableDebug()
{
}

int* TI83Plus::GetKeyMap()
{
    return keymap83;
}

int TI83Plus::GetKeyMat(int r,int c)
{
    return ti83kmat[r][c];
}

int TI83Plus::GetKeyAlphaMat(int r,int c)
{
...
void TI83Plus::Reset()
{
    ...
    port2=0; port4=0x76;
    mem[0]=rom; mem[1]=rom; mem[2]=rom; mem[3]=ram;
}

int TI83Plus::getmem(int addr)
{
    if ((addr>>20)==6)
        return readport(addr);
    if ((addr&0xffff8000)==VIDMEM_ADDR)
        return vidMem[addr&0xfff];
    return mem[(addr>>14)&3][addr&0x3fff];
/*	if ((word)addr<0x4000)
...
int TI83Plus::getmem_word(int addr)
{
	return ((getmem(addr+1)<<8)|getmem(addr))&0xffff;
}

int TI83Plus::getmem_dword(int addr)
{
...
void TI83Plus::setmem(int addr,int v)
{
    addr&=0xffffff;
    if ((mem[(addr>>14)&3]==ram)||(mem[(addr>>14)&3]==&ram[0x4000]))
        mem[(addr>>14)&3][addr&0x3fff]=v;
//	if ((word)addr>=0x8000)
//		ram[addr&0x7fff]=v;
}

void TI83Plus::setmem_word(int addr,int v)
{
	setmem(addr+1,(v>>8)&0xff);
	setmem(addr,v&0xff);
}

void TI83Plus::setmem_dword(int addr,int v)
{
...
int TI83Plus::readmem_word(int addr)
{
    if (debugCheckEnable)
    {
...
int TI83Plus::readmem_dword(int addr)
{
    if (debugCheckEnable)
    {
...
void TI83Plus::writemem(int addr,int v)
{
    if (debugCheckEnable)
    {
...
void TI83Plus::writemem_word(int addr,int v)
{
    if (debugCheckEnable)
    {
...
void TI83Plus::writemem_dword(int addr,int v)
{
    if (debugCheckEnable)
    {
...
int TI83Plus::readport(int port)
{
    int i,v;
    ...
    switch(port&0xff)
    {
        ...
        case 4:
            ...
            if ((!onpressed)&&(!onheld))
            {
                onpressed=1;
                maskOn=1;
                return (onheld<<3)|(R.IFF1?0:2)|maskOn;
            }
            if ((onpressed)&&(onheld))
                onpressed=0;
            return (onheld<<3)|(R.IFF1?0:2)|maskOn;
        case 6:
            return pageAType|pageANum;
        case 7:
...
void TI83Plus::writeport(int port,int v)
{
    ...
    switch(port&0xff)
    {
        case 0:
            ...
            if ((!sendInProgress)&&(!emuLink)&&
                ...
                ReceiveFile();
            else if ((!sendInProgress)&&(emuLink)&&
                ((outRed)||(outWhite))&&(!soundEnable))
            {
                sendInProgress=1;
                int v;
        ...
        case 3:
            ...
            break;
        case 4:
            if ((v==0x77)&&(port4!=0x77))
            {
                memcpy(&rom[0xf<<14],&rom[0x1f<<14],16384);
                mem[3]=mem[2];
                mem[1]=&rom[0xf<<14];
                mem[2]=&rom[0xf<<14];
                pageAType=0; pageANum=0xf;
                port4=v;
            }
            else if ((v==0x76)&&(port4!=0x76))
            {
                mem[1]=&rom[0x1f<<14];
                mem[2]=ram;
                mem[3]=ram;
                pageAType=0; pageANum=0x1f;
                pageBType=0x40; pageBNum=0;
                port4=v;
            }
            break;
        case 6:
/*            if (v>0x1f)
            ...
            pageAType=v&0x40;
            pageANum=pageAType?(v&1):(v&0x1f);
            pageA=pageAType?&ram[pageANum<<14]:&rom[pageANum<<14];
            mem[(port4==0x77)?2:1]=pageA;
            break;
        case 7:
            pageBType=v&0x40;
            pageBNum=pageBType?(v&1):(v&0x1f);
            pageB=pageBType?&ram[pageBNum<<14]:&rom[pageBNum<<14];
            mem[(port4==0x77)?3:2]=pageB;
            break;
        case 0x10:
//            if ((v&0xc0)==0xc0)
//                contrast=v&0x3f;
...
void TI83Plus::OnDown()
{
    onheld=0;
}

void TI83Plus::KeyUp(int r,int c)
{
    if (r!=-1)
        kbmatrix[r]|=1<<c;
}

void TI83Plus::KeyDown(int r,int c)
{
    if (r!=-1)
        kbmatrix[r]&=~(1<<c);
}

void TI83Plus::AlphaUp()
{
    KeyUp(5,7);
}

void TI83Plus::AlphaDown()
{
    KeyDown(5,7);
}
...
int TI83Plus::GetKeyID(int i)
{
    return key[i];
}

void TI83Plus::SaveState(char *fn)
{
    FILE *fp=fopen(fn,"wb");
    fprintf(fp,"VTIv2.1 ");
...
void TI83Plus::LoadState(char *fn)
{
    FILE *fp=fopen(fn,"rb");
    if (!fp) return;
...
int TI83Plus::SendReady()
{
    return 1;
}

int TI83Plus::SendBuffer(int c)
{
    return SendByte(c);
}

int TI83Plus::CheckBuffer()
{
...
int TI83Plus::SendFile(char *fn,int more)
{
    int i,hdr,csum,c,len,len2,len3,type,other,nameLen;
    char name[8];
    ...
    while (!feof(fp))
    {
        ...
        if (type==0xf)
        {
            ...
            if (!GetByte(c)) { fclose(fp); sendInProgress=0; return 0; }
            whiteWire=1; redWire=1; KBInt();
            SendByte(0x23); SendByte(0x56); SendByte(0); SendByte(0);
            SendByte(0x23); SendByte(0x15);
            SendByte(len&0xff); SendByte(len>>8);
            fseek(fp,2,SEEK_CUR);
            csum=0;
            for (i=0;i<len;i++)
        ...
        fread(name,8,1,fp);
        other=0; other=fread(&other,2,1,fp);
        fseek(fp,hdr-11,SEEK_CUR);
        whiteWire=1; redWire=1; KBInt();
        SendByte(0x23); SendByte(0xc9);
        SendByte(13); SendByte(0);
        SendByte(len&0xff); SendByte(len>>8);
        SendByte(type);
        ...
        SendByte(other&0xff); SendByte(other>>8);
        csum+=(other&0xff)+(other>>8);
...
int TI83Plus::ReceiveFile()
{
    ...
    if (c!=0x6) { sendInProgress=0; return 0; }
    ...
    while (1)
    {
        ...
        GetByte(c); GetByte(c);
        if (getError) { Screen->Cursor=crDefault; sendInProgress=0; getError=0; return 0; }
        newBuf=new char[bufLen+size+19];
        if (buf)
        {
        ...
        buf=newBuf;
        curOfs=bufLen+17;
        buf[bufLen]=0xd; buf[bufLen+1]=0;
        buf[bufLen+2]=size&0xff; buf[bufLen+3]=size>>8;
        buf[bufLen+4]=type;
        for (i=0;i<8;i++)
            buf[bufLen+5+i]=name[i];
        buf[bufLen+13]=1; buf[bufLen+14]=0;
        buf[bufLen+15]=size&0xff; buf[bufLen+16]=size>>8;
        for (i=0;i<size;i++)
        {
            GetByte(c);
            buf[curOfs+i]=c;
        ...
        if (!transferAsGroup)
        {
            ...
            dlog->DefaultExt=defExt8xp[type&0xf];
            if (name[0]==0x5c)
